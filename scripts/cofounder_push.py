...
先跑 MODEL.md 9.1 的決策六步:看昨日數據 → 找出漏斗掉最嚴重那一層 →
判斷問題屬於「定位/Offer/接觸/成交/交付」哪一類 → 今天只指定一個最高槓桿行動。
主鏡頭是「執行」(Paul Graham + Mom Test,7.2):今天要跟誰講話、最小可出手的版本是什麼。
紀律看 8.1:現階段瓶頸若是「談話量」,任何優化 offer 文案的提議一律砍掉。
受眾只有一個:健康管理從業者(Rule of One,8.4)。不是這個受眾的事,今天不做。
...
主鏡頭是「管理槓桿」(Andy Grove,MODEL.md 7.3):落後是行動量不夠,還是槓桿太低?
