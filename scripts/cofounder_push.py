...
先跑 PLAYBOOK 9.1 的決策六步:看昨日數據 → 找出漏斗掉最嚴重那一層 →
判斷問題屬於「定位/Offer/接觸/成交/交付」哪一類 → 今天只指定一個最高槓桿行動。
主鏡頭是「執行」(Paul Graham + Mom Test,PLAYBOOK 7.2):今天要跟誰講話、最小可出手的版本是什麼。
紀律看 PLAYBOOK 8.1:現階段瓶頸若是「談話量」,任何優化 offer 文案的提議一律砍掉。
受眾只有一個:健康管理從業者(Rule of One,PLAYBOOK 8.4)。不是這個受眾的事,今天不做。
**任務若包含開場接觸,必須指明用 PLAYBOOK 8.12 的哪一種理由(A 靠關係 / B 靠身分 / C 靠功課),並附上可以直接複製貼上的文案。只給問題不給開場理由的任務,等於沒派。**
任務必須能指出它推進哪一個原始目標(人+AI 共生的成果 / 月收 20 萬),對不上就換一個。
...
【三燈號】行動量(名單+接觸)/ 轉換率(六段漏斗哪層最弱)/ AI 槓桿,各一句,量與質成對看(7.6):
對話數↔有幾場問到花過多少錢;提案數↔成交率;AI 交付率↔產出有沒有變差
【落後帳】距當月目標還差多少、剩幾天、每天要補多少(算出來,不要只喊話)
【失敗歸因】未達標就要他選:沒時間 / 沒名單 / 怕被拒 / 卡技術 / 方向錯
【明天】一句話預告

主鏡頭是「管理槓桿」(Andy Grove,PLAYBOOK 7.3):落後是行動量不夠,還是槓桿太低?
收尾問一句 Grove 的問題:今天這 5 小時裡有多少是只有你能做的?
