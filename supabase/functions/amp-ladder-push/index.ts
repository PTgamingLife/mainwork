// 安麗蛋白素挑戰 — 爬梯子卡的一次性推播。
//
// 結構跟 amp-luck-push 一樣(同一組金鑰、同樣用 broadcast),差別只在送出的卡片。
...
//
// 部署:supabase functions deploy amp-ladder-push --no-verify-jwt

import { lineBroadcast, ladderCard, sbSelectRetry } from "../_shared/amp.ts";
...
const IMAGE_URL = Deno.env.get("AMP_LADDER_IMAGE_URL") || `${PAGES}/media/ladder.png`;
...
Deno.serve(async (req: Request) => {
  ...
  const appUrl = `${base}${base.includes("?") ? "&" : "?"}view=ladder`;
  const card = ladderCard({ imageUrl: IMAGE_URL, appUrl });
